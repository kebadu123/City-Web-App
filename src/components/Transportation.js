...
const Transportation = () => {
  ...
  useEffect(() => {
    fetchTransportationNews();
  }, []);

  const fetchTransportationNews = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/news/Transportation'); // Fetch Transportation news
      setNews(response.data);
    } catch (error) {
      console.error('Error fetching transportation news', error);
    }
  };
